...
async function resolveOpenPolymarketEvents(admin: ReturnType<typeof createAdminClient>) {
  const { data: openEvents, error: fetchError } = await admin
    .from("events").select("id, source_event_id").eq("status", "active")
    .eq("source_platform", "polymarket").not("source_event_id", "is", null);
  if (fetchError) throw new Error(fetchError.message);

  const resolved: string[] = [], stillOpen: string[] = [], needsReview: string[] = [];
  ...
  for (let i = 0; i < rows.length; i += CONCURRENCY) {
    const results = await Promise.all(rows.slice(i, i + CONCURRENCY).map(async (row) => {
      ...
      try {
        ...
        if (!market) return { kind: "error" as const, id: sourceId, error: "Not found on Polymarket" };
        ...
        const { error } = await admin.rpc("resolve_external_event", {
          p_source_platform: "polymarket", p_source_event_id: sourceId,
...
export async function GET(request: Request) {
  ...
  if (!checkCronAuth(request)) {
    console.error("[polymarket-cron] forbidden", { startedAt });
    return NextResponse.json({ error: "Forbidden", startedAt }, { status: 403 });
  }

  console.log("[polymarket-cron] started", { startedAt, topN: TOP_N });
  const admin = createAdminClient();
  let events;
  try {
    events = await fetchNewTopEvents(TOP_N);
    console.log("[polymarket-cron] fetched", { count: events.length });
  } catch (err) {
  ...
  const results = {
    ...
    resolved: [] as string[], stillOpen: [] as string[], needsReview: [] as string[],
    resolutionErrors: [] as { id: string; error: string }[],
  };

  for (const ev of events) {
    if (!ev.markets || ev.markets.length !== 1) { results.skipped.push(ev.id); continue; }
    ...
    try {
      const { data, error } = await admin.rpc("import_external_event", {
        p_source_platform: "polymarket", p_source_event_id: ev.id,
        p_source_condition_id: market.conditionId, p_title: ev.title,
        p_description: (ev.description || "").slice(0, 2000), p_category: mapCategory(ev.category),
        p_yes_price: prices.yes, p_no_price: prices.no, p_initial_liquidity_usd: initialLiquidity,
      });
      if (error) results.importErrors.push({ id: ev.id, error: error.message });
  ...
  const summary = {
    startedAt, completedAt, topN: TOP_N, fetched: events.length,
    imported: results.imported.length, skipped: results.skipped.length,
    importErrors: results.importErrors.length, resolved: results.resolved.length,
    stillOpen: results.stillOpen.length, needsReview: results.needsReview.length,
    resolutionErrors: results.resolutionErrors.length,
  };
